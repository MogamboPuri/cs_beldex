...
[`cs_beldex` package](https://pub.dev/packages/cs_beldex), 
the [`cs_beldex_flutter_libs` package](https://pub.dev/packages/cs_beldex), and their dependencies.

# About
- A simplified Flutter/Dart Beldex wallet library.
- Depends on https://github.com/MogamboPuri/monero_c/
- Abstracts the wallet2 spaghetti.
- Refactored and organized version of flutter_libbeldex based on
  https://github.com/cypherstack/flutter_libbeldex/tree/heavy-refactor.
...
By default, `cs_beldex_flutter_libs` will automatically include and download the
...
1. Add this repo as a git submodule to your project and add `cs_beldex`and 
   `cs_beldex_flutter_libs` to your pubspec.yaml as relative/path dependencies. 
   If you're migrating from `flutter_libbeldex` to `cs_beldex`, also add `compat`.
...
- This repo's build scripts are just wrappers around `monero_c`'s build scripts.
  For details and requirements see https://github.com/MogamboPuri/monero_c/
